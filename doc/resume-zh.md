...
- 实现多租户 HD 钱包模型（手写 BIP32 secp256k1/ed25519 派生），master seed 经 scrypt KDF + AES-256-GCM 加密存储于 LevelDB，预留 HSM(PKCS#11) 后端接口；签名后私钥即时清零
- 支持 ECDSA / EdDSA 双算法覆盖 Ethereum、BSC、Polygon、Arbitrum、Solana、Tron 等多链；设计提现异步队列与 stuck tx 自动加速
