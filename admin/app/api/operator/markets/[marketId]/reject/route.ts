...
export async function POST(request: Request, { params }: { params: Promise<{ marketId: string }> }) {
  ...
  const payload = (await request.json().catch(() => null)) as RejectRequestBody | null;
  if (!payload?.operator) {
    return NextResponse.json({ error: "operator payload is required" }, { status: 400 });
  }

  const message = `reject_market:${parsedMarketID}:${payload.operator.requestedAt}`;
  const auth = await authorizeOperatorAction(message, payload.operator);
  if (!auth.ok) {
    return auth.response;
  }

  const response = await forwardJSON(`/api/v1/admin/markets/${parsedMarketID}/reject`, {
    method: "POST",
    body: {
