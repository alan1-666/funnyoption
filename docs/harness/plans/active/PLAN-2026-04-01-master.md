...
| TASK-OFFCHAIN-019 | completed | worker | TASK-OFFCHAIN-018, TASK-CHAIN-025 | redesign the public market detail page around a Worm-style hierarchy while keeping FunnyOption lifecycle truth and trading behavior intact |
| TASK-OFFCHAIN-020 | completed | worker | TASK-OFFCHAIN-019 | remove meta/explanatory design copy from the detail page and codify one strict frontend surface-copy guideline |
...
| TASK-CHAIN-036 | completed | commander+worker | TASK-CHAIN-035 | finish the accepted/frozen closeout tranche by anchoring accepted escape-collateral roots, executing one operator-free frozen Merkle claim, preserving accepted claim runtime across rematerialization, switching financial read truth to accepted-first visibility, and upgrading the preferred proving lane to state-transition witness material |
